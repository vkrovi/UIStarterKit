...
extension Error {
    /// Cast a generic `Error` to a `NError`. If the receiver is already a `NError`, then it
...
public struct ExplicitError: NError {
    ...
    public init(code: ErrorCode, debugMessage: String? = nil) {
        self.code = code
...
public struct ErrorBox: Error, Equatable {
    public static func == (lhs: ErrorBox, rhs: ErrorBox) -> Bool {
        return lhs.nError.descriptionWithConditionalDebugMessage == rhs.nError.descriptionWithConditionalDebugMessage
    }
    
    var nError: NError
    init(_ sbtError: NError) { self.nError = sbtError }
}
